...
## Research 
This research has focused on the Olympic categories that Hungary performed at the top level since the earliest dates of Olympics when Hungary won a medal regardless of the medal rank. The main aim of the research is to identify the specific Olympic categories that Hungarian athletes tend to win higher-rank medals.
...
Since this dataset includes the world data, but not just Hungary, the team names are filtered for Hungarian teams to be used in the dataset. 
...
As the numbers below show, medal column of the dataset that has the highest number of missing values. height, weight and age are the columns following medal column for having missing values.
...
Since these missing values represent the results where Hungarian athletes did not win any medal and the main focus of this research is the Olympics categories where Hungarian athletes won at least a Bronze medal, missing values in medal columns have been removed from the dataset. In addition to medal, the age rows with missing values are also removed. 
```{r}
Hungary_olympics_data_medal <- Hungary_olympics_data %>%
    filter(!is.na(medal) & !is.na(age))
```

After the removal of missing values, current data shows that Hungarian athletes won medals in 20 different Olympic categories since the early days of Olympics.
```{r}
...
Since the category of Modern Pentathlon is actually combination of 5 different sub-categories, fencing, shooting, swimming, running, and horseback riding, rows with Modern Pentathlon have been excluded from the dataset. In addition to this change, similar sub-categories are combined in a ore general sub-category. For example, Judo, Karate, Boxing, Wrestling and Fencing" are combined under the new category Martial Arts. Water Polo, Swimming, Rowing, Sailing and Canoeing are united under a new sub-category called Water Sports. Tennis, Handball and Football are combined under the name of Ball Games. The subcategories of Athletics and Gymnastics are left untouched even though their attendence levels were lower than other categories. Other sub-categories where Hungary won only few medals like Art Competitions, Weightlifting, Equestrianism, Figure Skating and Shooting are combined under the category of Other. After these combinations, dataset ended up having 6 different sub-categories.
```{r}
...
## New Model
Since Water Sports, Martial Arts and Ball Games are top 3 sub-categories with the highest number of medals, I added Gymnastics and Athletics to the sub-group of Other and excluded from the model
...
This assumption requires having a parallel lines among the levels of dependent variable. The result of brant test below shows that our new model satisfies the proportional odds assumption. Additionally,  each of our independent variables also satisfy this assumption.
...
knitr::kable(result_table, caption = "Olympics Dataset - Ordered Logistic Regression", digits = 3)
...
knitr::kable(result_table_ci, caption = "Confidence Interval of Odd Ratios in Ordered Logistic Regression", digits = 3)
